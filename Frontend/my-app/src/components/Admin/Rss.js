...
export default function Rss() {
    ...
    const EditFeed = (id, _url, _topic, _publisher) => {
        ...
        setFeeds(feeds.map((feed) => feed.id === id ? {...feed, url: _url, topic: _topic, publisher: _publisher} : feed));
    ...
    return (
        <div className="container">
            <div className="row">
                <div className="col-md-auto offset-2">
                    ...
                    <ul className="list-group">
                        {filteredFeeds.map((feed, index) => (
                            <li className="list-group-item d-flex justify-content-between align-items-center"
                                key={feed.id}>
                                <p>{feed.url} > <small>{feed.topic}</small> <span
                                    className="badge bg-dark ms-2">{feed.publisher}</span></p>
